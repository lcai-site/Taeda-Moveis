...
const DashboardGrid: React.FC<DashboardGridProps> = ({ data, isLoading, error, aggregation, activeTab }) => {
  ...
  const ICONS = {
    ...
    cpl: <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>,
    rate: <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-teal-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z" /></svg>,
  };
  ...
  return (
    <>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 mb-8">
        <MetricCard title="Contatos no Dia" value={contactsToday} icon={ICONS.contacts} />
        <MetricCard title="Qualificados" value={totalQualified} icon={ICONS.qualified} description="No período" />
        <MetricCard title="Desqualificados" value={totalDisqualified} icon={ICONS.disqualified} description="No período" />
        <MetricCard title="Total Investido" value={`R$ ${totalCost.toFixed(2)}`} icon={ICONS.cost} description="No período" />
        <MetricCard title="Custo por Lead" value={`R$ ${avgCpl.toFixed(2)}`} icon={ICONS.cpl} description="Média no período" />
        <MetricCard title="Taxa de Qualif." value={`${qualificationRate.toFixed(1)}%`} icon={ICONS.rate} description="No período" />
      </div>
